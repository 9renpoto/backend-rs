...
use opensearch::{DeleteParts, GetParts, IndexParts, OpenSearch, SearchParts, UpdateParts, http::transport::{BuildError, SingleNodeConnectionPool, TransportBuilder}, indices::IndicesCreateParts};
...
async fn get_onsen(conn: web::Data<DBConnection>, path: web::Path<OnsenPath>) -> impl Responder {
    ...
    let result = conn
        ...
        .await;
    match result {
        Ok(onsen) => HttpResponse::Ok().json(&onsen),
        Err(e) => {
...
async fn create_onsen(conn: web::Data<DBConnection>, data: web::Json<Onsen>) -> impl Responder {
...
async fn update_onsen(
    ...
    if (&onsen.id)
        .as_ref()
        .filter(|id| id.to_string() == path.id)
        .is_none()
    {
...
async fn delete_onsen(conn: web::Data<DBConnection>, path: web::Path<OnsenPath>) -> impl Responder {
    println!("delete_onsen id: {}", &path.id);
    let result = conn
        .get_ref()
        .delete(DeleteParts::IndexId("onsen", path.id.as_str()))
        .send()
        .and_then(|r| async { r.json::<Document>().await })
        .await;
    match result {
        Ok(result) => HttpResponse::Ok().json(json!({ "id": result._id })),
        Err(e) => {
